...
vagram_no_bound_files = [f"tmp/vagram_no_bound_{i}.txt" for i in range(6)]

for i, (file_names, label) in enumerate(
    zip(
        [mse_files, vagram_files, vagram_full_files, vagram_no_bound_files],
        [
            "MSE",
            "VaGraM (1st order)",
            "VaGraM (2nd order)",
            "VaGraM (2nd order) unbounded",
        ],
    )
):
    rews = []
