...
def select_action(policy_network, theta, key, state, act_limit):
    ...
    a_tilde = mu + jnp.exp(log_std) * random.normal(key, shape=mu.shape)
    samples = jnp.tanh(a_tilde)

    # mu_log_probs = jnp.log(pdf_normal(samples, mu, jnp.exp(log_std)))
    mu_log_probs = jax.scipy.stats.norm.logpdf(samples, mu, jnp.exp(log_std))
    tanh_correction = jnp.sum(
        2.0 * (jnp.log(2.0) - a_tilde - jax.nn.softplus(-2.0 * a_tilde)),
        -1,
        keepdims=True,
    )
    samples = samples * act_limit
    log_prob = mu_log_probs - tanh_correction
    # print()
    # print(mu_log_probs)
    # print(tanh_correction)
    # print(log_prob.min())
    # print(log_prob.max())
    # print()
...
def select_mean_action(policy_network, theta, state, act_limit):
...
def compute_targets(
    ...
    a_tilde, gauss_log_prob = select_action(
        policy_network, theta, key, state, act_limit
    )
    target = (
        reward
        + gamma
        * (1 - done)
        * min_q_network(
            q_network, phi1, phi2, jnp.concatenate((state, a_tilde), axis=-1)
        )
...
def compute_q_loss(q_network, phi, inp, target):
    losses = (q_network.apply({"params": phi}, inp) - target) ** 2
