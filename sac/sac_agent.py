...
class SACAgent():
    ...
    def __init__(self, gamma, q_network_1, q_network_2, q_target_1, q_target_2, policy_network):
        self.gamma = gamma
        self.q_network_1 = q_network_1
        self.q_network_2 = q_network_2
        self.q_target_1 = q_target_1
        self.q_target_2 = q_target_2
        self.policy_network = policy_network

    def select_action(self, theta, key, state):
    ...
    def min_q_target(self, phi1, phi2, inp):
        return jax.lax.min(self.q_target_1.apply({'params': phi1}, inp), self.q_target_2.apply({'params': phi2}, inp))

    def compute_targets(self, phi1, phi2, reward, done, inp, action, alpha):
        return reward + self.gamma * (1 - done) * self.min_q_target(phi1, phi2, inp) - alpha * jnp.log(action)
    ...
    def compute_policy_loss(self, theta, phi1, phi2, state, alpha, key):
        a_tilde = self.select_action(theta, key, state)
